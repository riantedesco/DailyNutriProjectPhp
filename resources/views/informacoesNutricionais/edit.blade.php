...
@section('content')
    @foreach ($legendasNutricionais as $legendaNutricional)
        @if ($informacaoNutricional->legendaNutricional_id == $legendaNutricional->id)
            <div>
                <br>
                <h2 style="font-family: serif;">Editando informação nutricional:
                    {{ $legendaNutricional->descricao }}</h2>
                <hr size="2">
            </div>
            ...
            {!! Form::open([
                'route' => ['informacoesNutricionais.update', 'id' => $informacaoNutricional->id],
                'method' => 'put',
            ]) !!}
